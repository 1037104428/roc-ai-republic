...
| `verify-env-vars.sh` | 环境变量验证脚本 | 快速验证关键环境变量是否设置，支持完整验证和快速验证模式 |
...
| `../scripts/quick-verify-cdn-fallback.sh` | CDN回退策略验证 | 快速验证install-cn.sh的国内源回退策略和自检功能 |
...
| `quick-validation-examples.md` | 快速验证示例文档 | 提供各种场景下的快速验证示例和命令参考，位于docs目录 |
...
| `verify-prometheus-metrics.sh` | Prometheus监控指标验证脚本 | 验证quota-proxy的Prometheus监控指标功能，包含完整的指标收集测试 |
| `quick-verify-prometheus-monitoring.sh` | Prometheus监控快速验证脚本 | 快速验证Prometheus监控集成指南的完整性和可用性，检查文档、脚本和索引引用 |
