...
| 脚本名称 | 功能描述 | 验证目标 | 创建时间 |
|---------|---------|---------|---------|
| `verify-validation-index.sh` | 验证脚本索引文档验证 | 文档存在性、可读性、结构完整性、脚本引用 | 2026-02-11 |
...
```bash
# 1. 查看帮助信息
./scripts/verify-quota-proxy.sh --help

# 2. 模拟运行（不实际执行）
./scripts/verify-quota-proxy.sh --dry-run

# 3. 快速验证（只执行核心检查）
./scripts/verify-quota-proxy.sh --quick

# 4. 完整验证（执行所有检查）
./scripts/verify-quota-proxy.sh

# 5. 静默模式（只输出结果）
./scripts/verify-quota-proxy.sh --silent
```

### 示例2：验证数据库性能基准测试
```bash
# 查看所有可用选项
./scripts/verify-db-performance-benchmark.sh --help

# 快速检查脚本基本功能
./scripts/verify-db-performance-benchmark.sh --quick

# 详细模式查看所有测试细节
./scripts/verify-db-performance-benchmark.sh --verbose

# 生成HTML格式的报告
./scripts/verify-db-performance-benchmark.sh --html-report
```

### 示例3：批量验证多个脚本
```bash
# 验证所有核心基础设施脚本
for script in verify-quota-proxy.sh verify-admin-comprehensive.sh; do
    echo "=== 验证 $script ==="
    ./scripts/$script --quick
    echo ""
done

# 验证所有数据库相关脚本
for script in verify-db-performance-benchmark.sh verify-check-server-backup-status.sh; do
    echo "=== 验证 $script ==="
...
# 1. 验证环境配置
echo "1. 验证环境配置..."
./scripts/verify-env-config.sh --quick
...
# 4. 验证站点
echo "4. 验证站点..."
./scripts/verify-site-full-functionality.sh --quick
...
```bash
# 如果验证失败，使用详细模式查看具体问题
./scripts/verify-quota-proxy.sh --verbose

# 检查特定测试项
./scripts/verify-quota-proxy.sh --test health_check

# 跳过某些测试
./scripts/verify-quota-proxy.sh --skip container_check --skip network_check

# 查看测试报告摘要
./scripts/verify-quota-proxy.sh --summary
```
