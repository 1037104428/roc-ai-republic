...
拿 key 的方式见：`docs/posts/置顶_TRIAL_KEY_获取与使用_模板.md`（后续论坛上线后会作为置顶帖发布）。
...
```bash
export TRIAL_KEY="<把这里替换成你拿到的 TRIAL_KEY>"
```
...
```bash
# 仍然是 healthz，不会产生模型调用成本
curl -fsS ${OPENAI_BASE_URL}/healthz
```
...
OpenClaw 的安装方式以官方文档为准（不同系统略有差异）：

- 文档入口：`docs/links.md`

### 4.2 配置 OpenClaw 使用本网关

确保你在同一个终端里已经设置：

- `OPENAI_API_KEY=$TRIAL_KEY`
- `OPENAI_BASE_URL=https://api.clawdrepublic.cn`

然后运行任意一次最小任务（示例，具体以你本机的 OpenClaw 命令为准）：

```bash
openclaw --help
```

如果你能看到帮助信息，说明 OpenClaw 已安装。

接下来让 OpenClaw 发起一次模型调用（示例）：
