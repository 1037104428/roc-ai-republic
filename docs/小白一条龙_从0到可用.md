...
（可选）如果你是“仓库贡献者/开发者”，并且已经把本仓库克隆到了本地，可以做一次“全链路探活”（官网/API/安装脚本等，不会调用模型）：

```bash
# 在你本地的 roc-ai-republic 仓库根目录执行
./scripts/probe-roc-all.sh --timeout 8
```

### 4.2 让 OpenClaw 使用本网关（推荐用法）
...
如果你看到默认模型指向 `clawd-gateway/...`（或你配置的 provider），说明已跑通。遇到问题，把 `openclaw models status` 输出 + 时间贴到论坛求助帖里即可。
