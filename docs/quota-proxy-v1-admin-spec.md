...
- 字段说明：
  - `day`：按天聚合的日期（`YYYY-MM-DD`）。
  - `mode`：后端存储模式/实现路径（例如 `file`/`sqlite`），用于运维排查。
  - `items[]`：用量条目列表。
    - `key`：trial key（可能会在 UI/日志里做脱敏展示）。
    - `req_count`：该 key 在当天累计的请求次数（成功/失败是否计入由实现定义，需在实现 README 明确）。
    - `updated_at`：最后一次写入/更新用量的时间戳（毫秒）。

> 兼容模式（用于快速排查最近用量）：不带 `day` 时，可用 `?limit=50` 返回跨天的最近记录（字段含 `day`），建议只在运维排查使用。
